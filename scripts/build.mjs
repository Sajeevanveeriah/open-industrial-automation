...
for(const file of ['styles.css','app.mjs']){const hash=createHash('sha256').update(await readFile(resolve(target,file))).digest('hex').slice(0,12);html=html.replace(file+'?v=potato-only-4',file+'?v=potato-only-4-'+hash);}
...
for (const alias of ['', 'suite/']) {
 ...
 for (const file of ['runtime-bootstrap.js','app.js','product-shell.js']) {
  await writeFile(resolve(target,alias,file),`location.replace(new URL('${back}?release=potato-only-4#plant',document.currentScript?.src || document.baseURI).href);\n`);
 }
}
await writeFile(resolve(target,'release.json'),JSON.stringify({release:'potato-only-4',commit:process.env.GITHUB_SHA||'local'}));
