...
try{
 ...
 await openRoute('Scenario studio');await page.locator('.fault-row').filter({hasText:'Emergency stop'}).getByRole('button',{name:'Remove cause'}).click();await page.getByRole('button',{name:'Reset trip',exact:true}).click();assert.match(await page.locator('.transport').innerText(),/HELD/);
 ...
 for(const width of [390,768,1440]){await page.setViewportSize({width,height:1000});for(const name of routes){await openRoute(name);assert.equal(await page.locator('body').evaluate(e=>e.scrollWidth>innerWidth),false,name+' overflow at '+width);}}
